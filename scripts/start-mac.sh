#!/bin/bash
# Start the LegalDraft application on macOS.
#
# Usage:
#   ./scripts/start-mac.sh           # Docker mode (default) — requires Docker Desktop
#   ./scripts/start-mac.sh --dev     # Local dev mode — runs Next.js + uvicorn directly
...
if [[ "$1" == "--dev" ]]; then
    ...
    if [ -f "$PROJECT_ROOT/.backend.pid" ] || [ -f "$PROJECT_ROOT/.frontend.pid" ]; then
        echo "Error: dev servers appear to already be running. Run './scripts/stop-mac.sh --dev' first."
        exit 1
    fi
    ...
    echo ""
    echo "Stop with: ./scripts/stop-mac.sh --dev"
else
